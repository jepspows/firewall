...
SIGNATURES: list[dict] = [
    ...
    # ── Token Smuggling ──
    {
        ...
        "patterns": [
            r"(?i)(ignore|skip|bypass)\s+(the\s+)?(above|previous)\s+(and|&)\s+(do|execute|run)",
            r"\[END\].*\[BEGIN\]",
    ...
    # ── Data Exfiltration ──
    {
        ...
        "patterns": [
            r"(?i)send\s+(this|the\s+(conversation|chat|transcript|output))\s+to",
            r"(?i)(encode|base64|hex)\s+(the\s+)?(response|output|answer)",
            r"(?i)summarize\s+the\s+(conversation|chat)\s+(so\s+far|history)",
        ],
        "confidence": 0.75,
    ...
    # ── Multi-turn Attack Setup ──
    {
        ...
        "patterns": [
            r"(?i)(remember|store|save)\s+this\s+(for|later)",
